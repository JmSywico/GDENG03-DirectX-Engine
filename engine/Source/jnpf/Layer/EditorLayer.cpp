...
namespace
{
	...
	const char* PrimitiveName(jnpf::Scene::PrimitiveType type)
	{
		switch (type)
		{
		case jnpf::Scene::PrimitiveType::Cube: return "Cube";
		case jnpf::Scene::PrimitiveType::Rectangle: return "Rectangle";
		case jnpf::Scene::PrimitiveType::Pyramid: return "Pyramid";
		case jnpf::Scene::PrimitiveType::Plane: return "Plane";
		case jnpf::Scene::PrimitiveType::Sphere: return "Sphere";
	...
	, m_viewportStateCallback(std::move(viewportStateCallback))
{
	...
	m_context.InstantiatePrefab = [this](
		...
		const DirectX::XMFLOAT3* rootPosition)
		{
			...
			jnpf::Editor::PrefabAsset prefab;
			if (!LoadPrefab(path, prefab)) return false;
		const std::uint64_t parentID = requestedParentID;
		auto command = std::make_unique<jnpf::Editor::InstantiatePrefabCommand>(
		...
		const DirectX::XMFLOAT3* position)
	{
		...
		const jnpf::Editor::EntitySnapshot snapshot =
			jnpf::Editor::EntitySnapshot::CaptureSubtree(*m_context.ActiveScene, entity);
		m_commandStack.RecordExecuted(std::make_unique<jnpf::Editor::AdoptEntityCommand>(
	...
	m_context.SaveAsPrefab = [this](entt::entity root)
	{
		...
		if (const auto* tag = m_context.ActiveScene->GetComponent<jnpf::Scene::TagComponent>(root))
			name = tag->Tag;
		...
		std::string error;
		if (!jnpf::Editor::PrefabAsset::SaveFromScene(
		...
		m_context.Assets->Register(path, jnpf::Graphics::AssetType::Prefab);
	...
	m_context.CountPrefabReferences = [this](std::uint64_t handle)
	{
		...
		for (const entt::entity entity : m_context.ActiveScene->GetAllEntities())
		{
			const auto* prefab = m_context.ActiveScene->GetComponent<
				jnpf::Scene::PrefabInstanceComponent>(entity);
	...
	m_context.DeleteAsset = [this](std::uint64_t handle, const std::string& path)
	{
		...
		const jnpf::Graphics::AssetReference asset = m_context.Assets->GetReference(handle);
		if (!asset.IsValid()) return false;
		m_commandStack.Execute(std::make_unique<jnpf::Editor::DeleteAssetCommand>(
...
void EditorLayer::OnEvent(Event& event)
{
	...
	if (keyEvent->Key == VK_F6 && m_context.GetSimulationState)
	{
		...
		if (state == jnpf::Editor::SimulationState::Stopped)
		{
	...
	{
	case 'W':
		m_context.CurrentGizmoMode = jnpf::Editor::GizmoMode::Translate;
		break;
	case 'E':
		m_context.CurrentGizmoMode = jnpf::Editor::GizmoMode::Rotate;
		break;
	case 'R':
		m_context.CurrentGizmoMode = jnpf::Editor::GizmoMode::Scale;
		break;
	case VK_DELETE:
		DeleteSelection();
		break;
...
void EditorLayer::OnImGuiRender()
{
	...
	const bool simulationActive = m_context.GetSimulationState
		&& m_context.GetSimulationState() != jnpf::Editor::SimulationState::Stopped;
...
void EditorLayer::ApplyWorkbenchStyle()
{
	...
	style.WindowRounding = jnpf::Editor::UI::Scale(2.0f);
	style.ChildRounding = jnpf::Editor::UI::Scale(2.0f);
	style.FrameRounding = jnpf::Editor::UI::Scale(2.0f);
	style.PopupRounding = jnpf::Editor::UI::Scale(2.0f);
	style.GrabRounding = jnpf::Editor::UI::Scale(2.0f);
	style.TabRounding = jnpf::Editor::UI::Scale(2.0f);
	style.WindowBorderSize = jnpf::Editor::UI::Scale(1.0f);
	style.FrameBorderSize = 0.0f;
	style.WindowPadding = jnpf::Editor::UI::Scale(10.0f, 8.0f);
	style.FramePadding = jnpf::Editor::UI::Scale(7.0f, 4.0f);
	style.ItemSpacing = jnpf::Editor::UI::Scale(8.0f, 5.0f);
	style.CellPadding = jnpf::Editor::UI::Scale(6.0f, 4.0f);
...
void EditorLayer::DrawDockspace()
{
	...
	const float chromeHeight = jnpf::Editor::UI::Scale(32.0f);
...
void EditorLayer::DrawViewportWindows()
{
	...
	const float availableWidth = std::max(jnpf::Editor::UI::Scale(640.0f),
		static_cast<float>(windowSize.x) - jnpf::Editor::UI::Scale(648.0f));
	const float halfHeight = std::max(jnpf::Editor::UI::Scale(220.0f),
		(static_cast<float>(windowSize.y) - jnpf::Editor::UI::Scale(132.0f)) * 0.5f);
		...
		if (visible)
		{
			...
			if (acceptPrefabDrop && ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(
					jnpf::Editor::AssetDragPayloadID,
					ImGuiDragDropFlags_AcceptBeforeDelivery))
				{
					const auto& asset = *static_cast<const jnpf::Editor::AssetDragPayload*>(payload->Data);
					if (asset.Type == jnpf::Graphics::AssetType::Prefab
						|| asset.Type == jnpf::Graphics::AssetType::Model)
					{
						const ImVec2 mouse = ImGui::GetMousePos();
	...
	drawViewport(
		"Scene",
		jnpf::Editor::UI::Scale(12.0f, 48.0f),
	...
	drawViewport(
		"Game",
		jnpf::Editor::UI::Scale(12.0f, 48.0f),
		ImVec2(std::max(jnpf::Editor::UI::Scale(320.0f), static_cast<float>(windowSize.x) * 0.32f), halfHeight),
...
void EditorLayer::SynchronizeActiveScene(const std::vector<std::uint64_t>* preservedSelection)
{
	...
	jnpf::Scene::Scene* nextScene = &m_activeSceneProvider();
...
bool EditorLayer::LoadPrefab(
	const std::string& path,
	jnpf::Editor::PrefabAsset& result) const
{
	if (!m_context.Assets) return false;
	const auto primitiveResolver = [this](const jnpf::Scene::PrimitiveDesc& desc)
	{
		jnpf::Scene::Scene temporary("PrefabModelResolver");
		const entt::entity entity = jnpf::Scene::CreatePrimitive(
			temporary, desc, m_context.CreatePrimitiveModel);
		const auto* renderer = temporary.GetComponent<jnpf::Scene::MeshRendererComponent>(entity);
	...
	return jnpf::Editor::PrefabAsset::Load(
	...
	const DirectX::XMFLOAT3* position)
{
	...
	jnpf::Scene::TransformComponent transform;
	if (position) transform.SetLocalPosition(*position);
	scene.AddComponent<jnpf::Scene::TransformComponent>(entity, transform);
	...
	scene.AddComponent<jnpf::Scene::MeshRendererComponent>(entity, renderer);
	scene.AddComponent<jnpf::Scene::RenderSourceComponent>(entity, {
		.Type = jnpf::Scene::RenderSourceType::ImportedModel,
		.AssetHandle = handle,
		.AssetPath = path
	});
	if (parentID != 0)
	...
	const DirectX::XMFLOAT3& position)
{
	...
	if (m_assetPreviewRootID == 0)
	{
		...
		if (asset.Type == jnpf::Graphics::AssetType::Model)
		{
			root = CreateModelAssetEntity(asset.Handle, asset.Path.data(), 0, &position);
		}
		else if (asset.Type == jnpf::Graphics::AssetType::Prefab)
		{
			jnpf::Editor::PrefabAsset prefab;
		...
		{
			scene.AddComponent<jnpf::Scene::TransientEditorComponent>(entity);
...
void EditorLayer::CommitAssetPreview()
{
	...
	{
		scene.RemoveComponent<jnpf::Scene::TransientEditorComponent>(entity);
		for (const entt::entity child : scene.GetChildren(entity)) self(self, child);
	};
	removeTransient(removeTransient, root);
	const auto snapshot = jnpf::Editor::EntitySnapshot::CaptureSubtree(scene, root);
	m_commandStack.RecordExecuted(std::make_unique<jnpf::Editor::AdoptEntityCommand>(scene, snapshot));
	m_context.SelectEntity(root);
	m_assetPreviewRootID = 0;
	m_assetPreviewHandle = 0;
	m_assetPreviewType = jnpf::Graphics::AssetType::Unknown;
}
...
void EditorLayer::DrawSceneContextPopup()
{
	...
	ImGui::SetNextWindowSizeConstraints(jnpf::Editor::UI::Scale(190.0f, 0.0f), ImVec2(FLT_MAX, FLT_MAX));
	...
	const auto createEntity = [&](std::unique_ptr<jnpf::Editor::CreateEntityCommand> command)
	{
		jnpf::Editor::CreateEntityCommand* created = command.get();
		m_commandStack.Execute(std::move(command));
		selectCreated(created->GetEntityID());
		return created->GetEntityID();
	};
	const auto createPrimitive = [&](jnpf::Scene::PrimitiveType type)
	{
		if (!m_context.CreatePrimitiveModel)
			return;

		jnpf::Scene::PrimitiveDesc desc;
		desc.Type = type;
		desc.Name = PrimitiveName(type);
		desc.Position = m_sceneContextPosition;
		if (type == jnpf::Scene::PrimitiveType::Rectangle)
		{
			desc.Width = 2.0f;
			desc.Height = 1.0f;
			desc.Depth = 1.0f;
		}
		else if (type == jnpf::Scene::PrimitiveType::Plane)
		{
			desc.Width = 8.0f;
			desc.Depth = 8.0f;
		}

		auto command = std::make_unique<jnpf::Editor::CreatePrimitiveCommand>(
			scene,
			desc,
			m_context.CreatePrimitiveModel);
		jnpf::Editor::CreatePrimitiveCommand* created = command.get();
		m_commandStack.Execute(std::move(command));
		selectCreated(created->GetEntityID());
	};
	...
	if (ImGui::MenuItem("Empty Element"))
		createEntity(std::make_unique<jnpf::Editor::CreateEntityCommand>(scene, "Element"));
	if (ImGui::MenuItem("Camera"))
	{
		const std::uint64_t id = createEntity(std::make_unique<jnpf::Editor::CreateEntityCommand>(
			scene,
			"Camera",
			jnpf::Scene::CameraComponent{}));
		if (scene.GetSettings().ActiveCameraEntityID == 0)
			m_commandStack.Execute(std::make_unique<jnpf::Editor::SetActiveCameraCommand>(
				scene,
				0,
				id));
	}
	if (ImGui::MenuItem("Light"))
	{
		const std::uint64_t id = createEntity(std::make_unique<jnpf::Editor::CreateEntityCommand>(
			scene,
			"Light",
			jnpf::Scene::LightComponent{}));
		if (scene.GetSettings().ActiveLightEntityID == 0)
			m_commandStack.Execute(std::make_unique<jnpf::Editor::SetActiveLightCommand>(
				scene,
				0,
				id));
	}
	if (ImGui::BeginMenu("3D Object", m_context.CreatePrimitiveModel != nullptr))
	{
		if (ImGui::MenuItem("Cube"))
			createPrimitive(jnpf::Scene::PrimitiveType::Cube);
		if (ImGui::MenuItem("Rectangle"))
			createPrimitive(jnpf::Scene::PrimitiveType::Rectangle);
		if (ImGui::MenuItem("Pyramid"))
			createPrimitive(jnpf::Scene::PrimitiveType::Pyramid);
		if (ImGui::MenuItem("Plane"))
			createPrimitive(jnpf::Scene::PrimitiveType::Plane);
		if (ImGui::MenuItem("Sphere"))
			createPrimitive(jnpf::Scene::PrimitiveType::Sphere);
		ImGui::EndMenu();
	}
...
void EditorLayer::DrawLightWorkbench()
{
	...
	if (ImGui::BeginTable("##LightRows", 5, flags, jnpf::Editor::UI::Scale(0.0f, 220.0f)))
	{
		ImGui::TableSetupScrollFreeze(0, 1);
		ImGui::TableSetupColumn("On", ImGuiTableColumnFlags_WidthFixed, jnpf::Editor::UI::Scale(34.0f));
		ImGui::TableSetupColumn("Element", ImGuiTableColumnFlags_WidthStretch);
		ImGui::TableSetupColumn("Type", ImGuiTableColumnFlags_WidthFixed, jnpf::Editor::UI::Scale(72.0f));
		ImGui::TableSetupColumn("Energy", ImGuiTableColumnFlags_WidthFixed, jnpf::Editor::UI::Scale(62.0f));
		ImGui::TableSetupColumn("Shadow", ImGuiTableColumnFlags_WidthFixed, jnpf::Editor::UI::Scale(62.0f));
...
void EditorLayer::DrawGizmo()
{
	...
	const auto* transform = m_context.ActiveScene->GetComponent<jnpf::Scene::TransformComponent>(
...
void EditorLayer::BeginGizmoDrag()
{
	...
	for (const entt::entity entity : m_context.GetSelectedEntities())
	{
		...
		const auto* selectedTransform =
			m_context.ActiveScene->GetComponent<jnpf::Scene::TransformComponent>(entity);
...
void EditorLayer::CommitGizmoDrag()
{
	if (!m_gizmoDragActive)
		return;
	auto command = std::make_unique<jnpf::Editor::CompositeCommand>();
	for (const GizmoDragEntry& entry : m_gizmoDragEntries)
	{
		...
		if (!transform)
			continue;
		const jnpf::Editor::TransformValue after{
			transform->GetLocalPosition(),
		...
		if (changed)
		{
			command->Add(std::make_unique<jnpf::Editor::SetTransformCommand>(
...
void EditorLayer::DuplicateSelection()
{
	...
	auto command = std::make_unique<jnpf::Editor::CompositeCommand>();
	std::vector<jnpf::Editor::DuplicateEntityCommand*> duplicates;
	for (const entt::entity entity : roots)
	{
		auto duplicate = std::make_unique<jnpf::Editor::DuplicateEntityCommand>(
			*m_context.ActiveScene,
			entity);
	...
	for (const entt::entity candidate : m_context.GetSelectedEntities())
	{
		...
		if (!nested)
			m_entityClipboard.push_back(
				jnpf::Editor::EntitySnapshot::CaptureSubtree(*m_context.ActiveScene, candidate));
	}
}
...
void EditorLayer::PasteSelection()
{
	if (!m_context.ActiveScene || !m_context.Commands || m_entityClipboard.empty())
		return;
	auto command = std::make_unique<jnpf::Editor::CompositeCommand>();
	std::vector<jnpf::Editor::PasteEntityCommand*> pasted;
	for (const auto& snapshot : m_entityClipboard)
	{
		auto item = std::make_unique<jnpf::Editor::PasteEntityCommand>(
			*m_context.ActiveScene, snapshot);
...
void EditorLayer::DeleteSelection()
{
	...
	for (const entt::entity candidate : m_context.GetSelectedEntities())
	{
		...
		if (!nested)
			command->Add(std::make_unique<jnpf::Editor::DeleteEntityCommand>(
				*m_context.ActiveScene,
				candidate));
	}
	if (!command->Empty())
