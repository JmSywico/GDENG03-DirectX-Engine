...
namespace
{
	const char* EntityName(const jnpf::Scene::Scene& scene, entt::entity entity, std::string& fallback)
	{
		if (const auto* tag = scene.GetComponent<jnpf::Scene::TagComponent>(entity);
			tag && tag->Tag[0] != '\0')
			return tag->Tag;
	...
	void Reparent(
		jnpf::Editor::EditorContext& context,
		entt::entity child,
		entt::entity parent)
	{
		...
		if (!context.Commands || !scene.CanSetParent(child, parent))
			return;
		context.Commands->Execute(std::make_unique<jnpf::Editor::SetParentCommand>(
			scene,
	...
	void DrawCreateMenu(jnpf::Editor::EditorContext& context, entt::entity parent)
	{
		...
		if (ImGui::MenuItem("Empty Element"))
			createEntity(std::make_unique<jnpf::Editor::CreateEntityCommand>(
				scene,
				"Element",
				parentID));
		if (ImGui::MenuItem("Camera"))
		{
			...
			if (scene.GetSettings().ActiveCameraEntityID == 0)
				context.Commands->Execute(std::make_unique<jnpf::Editor::SetActiveCameraCommand>(
					scene,
					0,
					id));
		}
		if (ImGui::MenuItem("Light"))
		{
			...
			if (scene.GetSettings().ActiveLightEntityID == 0)
				context.Commands->Execute(std::make_unique<jnpf::Editor::SetActiveLightCommand>(
					scene,
					0,
					id));
		}
	...
	std::vector<entt::entity> TopLevelSelection(jnpf::Editor::EditorContext& context)
	{
	...
	void DuplicateSelection(jnpf::Editor::EditorContext& context)
	{
		if (!context.Commands)
			return;
	...
	void DeleteSelection(jnpf::Editor::EditorContext& context)
	{
		if (!context.Commands)
			return;
		auto command = std::make_unique<jnpf::Editor::CompositeCommand>();
	...
	{
		auto& scene = *context.ActiveScene;
		if (scene.HasComponent<jnpf::Scene::TransientEditorComponent>(entity)) return;
		std::string fallback;
		...
		if (ImGui::BeginDragDropSource())
		{
			...
			ImGui::SetDragDropPayload("JNPF_ENTITY_ID", &id, sizeof(id));
			ImGui::TextUnformatted(name);
			ImGui::EndDragDropSource();
		}
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("JNPF_ENTITY_ID"))
			{
				...
				jnpf::Editor::AssetDragPayloadID))
			{
				...
				if (asset.Type == jnpf::Graphics::AssetType::Prefab && context.InstantiatePrefab)
					context.InstantiatePrefab(
						asset.Handle, asset.Path.data(), scene.GetEntityID(entity), nullptr);
				else if (asset.Type == jnpf::Graphics::AssetType::Model && context.InstantiateModel)
